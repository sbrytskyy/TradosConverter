...
void debug_settings::load(const std::string &filename)
{
	...
	BOOST_FOREACH(const PT::ptree::value_type& node, pt.get_child("debug.modules"))
	{
		std::cout << "node.first: " << node.first;
		std::cout << ", node.second: " << node.second.data() << std::endl;
	}

	BOOST_FOREACH(const PT::ptree::value_type& node, pt.get_child("debug"))
	{
		...
		if (node.first == "Node")
		{
			/*std::cout << "node.first: " << node.first;
			std::cout << ", node.second: " << node.second.data() << std::endl;*/
			const PT::ptree& nodeTree = node.second;
